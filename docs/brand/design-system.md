...
named layer that sits *before* Tailwind's `components`/`utilities` layers:
...
*thing* — a card, a chip, the nav, an icon button. `--border-input` is quieter than
...
A page where every link is emerald by default has no way to make *one* link mean something — the
...
display, H1, and H2, plus title-role text set at the H3 *size* — entry, item, and card titles, the
...
A floating pill, not a full-width bar pinned to the edge — the page's content sits *on* a surface
rather than *under* a toolbar. 64px tall (56px below 720px), `--radius-xl`, sticky 1rem from the
...
section changed while scrolling). There is no hamburger menu at any width: `.nav-links` scrolls
horizontally with its scrollbar hidden, so the full link set stays one tap away instead of one tap
plus a menu, and the bar keeps the same height and shape at every viewport width. The lockup never
shrinks or wraps (`flex: 0 0 auto; white-space: nowrap`) even when the nav is tight for space.
...
Two conventions, both hand-authored inline SVG rather than an icon package — a fixed, small set of
marks that never changes costs less to draw once than to pull in a dependency to render six of
them.

**Filled marks** — brand/social glyphs (GitHub, LinkedIn, X, YouTube, Instagram, Facebook): 24×24
viewBox, `fill="currentColor"`, no stroke, path data lives in `lib/social-icons.ts`. Rendered at an
18px glyph inside a 20px hit area, `--fg-muted` at rest, stepping to `--brand` on hover — never
emerald at rest, only as a state.

**Line icons** — interface actions (theme toggle, external-link indicator, the "view all" arrow,
the contact-email envelope): `fill="none"`, `stroke="currentColor"`, `stroke-linecap="round"`,
stroke-width 1.7–2 depending on size (lighter stroke at larger sizes, heavier at smaller, so
optical weight stays consistent — 1.7 at 16px, 1.8 at 18px, 2 at 14px). Sizes in use: 14, 16, 18,
20px; no icon renders smaller than 14px, where a 1.7–2px stroke stops reading as a line.

**Every decorative icon carries `aria-hidden="true"`.** The accessible name comes from the
surrounding link's text or an explicit `aria-label` on the control (`aria-label="Switch colour
theme"` on the toggle, `aria-label={icon.label}` on each social link) — never from the icon itself.

**Known gap:** `components/blog/Share.tsx` draws its share-link icons from the `react-icons`
package (`Fi`/`Fa6` sets) rather than hand-authored paths — the one place in the app that departs
from this system. Treat it as a gap to close when that component is next touched, not as a second
sanctioned icon pattern.
...
A component with a visible border never gets both the hard outline *and* the soft ring — they'd
...
**Container:** one measure for the whole site, `min(100% - 2rem, 760px)`. The blog reader is the
one deliberate exception — `min(100% - 2rem, 1020px)` — because the 760px measure plus a 220px
table-of-contents rail needs more room than the rest of the site's pages do; it isn't a second,
independently-chosen width.

**One breakpoint: 720px**, as a `max-width` query (the system overrides desktop-first rather than
enhancing mobile-first). Below it: the display steps down to 34px, the nav drops to 56px and tucks
its padding in, the hero/entry/item grids collapse to one column, the gallery drops to two columns,
and item metadata left-aligns instead of right-aligning.
...
minimum width is its *content's* min-content width — so a fixed-width sidebar next to a flexible
...
```css
:root, [data-theme="dark"]  { /* Carbon foundation, Emerald Bright */ }
[data-theme="light"]        { /* Paper foundation, Emerald Deep */ }
@media (prefers-color-scheme: light) {
  :root:not([data-theme]) { /* system light, no explicit choice */ }
}
```
